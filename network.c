...
[TERMINATE <connection id>] - terminate the connection on LIST\n\n\
...
[DISPLAY] - display information\n[name, UBIT name, UB e-mail, IP address of the process, port this process is listening.\n\n\
[LIST] - Display numbered list of all the connections this process if part of\n\n\
[TERMINATE <connection id>] - terminate the connection on the LIST\n\n\
[QUIT] - CLOSE all connections and terminate this process \n\n";
char* display = 
"Author : ISSAC CHOI \n\
UBIT : issaccho\n\
		...
		//decreade fd_count because we know something will be done in here.
			...
			//if it is from keyboard,
				...
				if(tokens[0] == '\0'){
					continue;
				//empty input.. ignore
				}else if(strcasecmp(tokens[0], "HELP") ==0 && numTok == 1){
					printf(serv_usage);
				...
				}else if(strcasecmp(tokens[0], "LIST") ==0 && numTok ==1){
					LIST();
				...
				}else if(strcasecmp(tokens[0], "TERMINATE") == 0 && numTok == 2){
				...
				//from first client index of serv_IP
					...
					//if the socket is triggered..
						...
						//if there is incoming..
							...
							//parse token.
							if(strcasecmp(tokens[0], "LIST")==0 && numTok ==1){
								send(serv_IP[t].sock,list,MAX_LIST,0);
								//send current client list to the client!
							}else if(strcasecmp(tokens[0], "CONNECT")==0 && numTok ==3){
								int ip,i,p;
		...
		//wait until there is an input and starts to work at coming inputs.
			...
			//if it is from keyboard,
				...
				if(tokens[0] == '\0'){
					...
					printf(" %s \n",getLOIP());
					printf("Income port : %d", income_port);
				}else if(strcasecmp(tokens[0], "REGISTER") == 0 && numTok ==3){
				...
				}else if(strcasecmp(tokens[0], "SEND") == 0 && numTok >= 3){
					if(REGISTERED==TRUE){
						int i;
						message[0]='\0';
						//init message
